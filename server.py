...
if not DNS_AVAILABLE:
    log.error("dnspython not installed — DNS lookups will fail. Run: pip install dnspython")
...
def domain_exists(domain):
    if not DNS_AVAILABLE: return True  # assume exists, let SMTP fail naturally
...
def server_error(e):
    r = jsonify({"error": "internal server error", "detail": str(e)})
    r.headers["Access-Control-Allow-Origin"] = "*"
    return r, 500

...
# Allow ALL origins including file:// (null origin) for local HTML usage
# Allow ALL origins — including GitHub Pages and file:// (null origin)
CORS(app, resources={r"/*": {"origins": "*"}})

@app.after_request
def add_cors_headers(response):
    # Hardcode wildcard on EVERY response — overrides any Flask-CORS defaults
    response.headers["Access-Control-Allow-Origin"]  = "*"
    response.headers["Access-Control-Allow-Headers"] = "Content-Type, Authorization, Accept, X-Requested-With"
    response.headers["Access-Control-Allow-Methods"] = "GET, POST, OPTIONS, HEAD"
    response.headers["Access-Control-Max-Age"]       = "86400"
    # Remove any restrictive header that may have been set upstream
    response.headers.remove("Access-Control-Allow-Credentials") if "Access-Control-Allow-Credentials" in response.headers else None
    return response

@app.before_request
def handle_options():
    """Handle OPTIONS preflight immediately — before any route logic."""
    if request.method == "OPTIONS":
        from flask import make_response
        r = make_response("", 204)
        r.headers["Access-Control-Allow-Origin"]  = "*"
        r.headers["Access-Control-Allow-Headers"] = "Content-Type, Authorization, Accept, X-Requested-With"
...
def index():
    """Serve the frontend HTML — so the Railway URL works directly in browser."""
    base = os.path.dirname(os.path.abspath(__file__))
    if os.path.exists(os.path.join(base, "index.html")):
        return send_from_directory(base, "index.html")
    return jsonify({"status": "ok", "service": "MailCheck v3 — frontend not found"})
